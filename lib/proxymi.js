...
(global =>
{
    ...
    function classesImpl(types)
    {
        if (types.length)
        {
            // Fail if a type is not a constructor or if its property 'prototype' has a value that
            // is not object.
            types.forEach(
                type =>
                class extends type
    ...
    function hasInstance(value)
    {
        const result =
            isCallable(this) &&
            !isPrimitive(value) &&
            (function_hasInstance.call(this, value) || isInPrototypeChain(this.prototype, value));
        return result;
    }
    ...
    function installHasInstance(types)
    {
        ...
        {
            if (isCallable(type) && type !== function_prototype)
            {
                installed = true;
