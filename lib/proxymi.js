...
(function (global)
{
    ...
    function createProxy(target, prototypeList)
    {
        ...
        let handler =
        {
            ...
            {
                let obj = objs.find(obj => propName in obj);
                if (obj != null)
                {
                    let value = reflect_get(obj, propName, receiver);
                    return value;
                }
            },
            has: (target, propName) => objs.some(obj => propName in obj),
            set(target, propName, value, receiver)
            {
                let obj = objs.find(obj => propName in obj);
                if (obj != null)
                {
                    let success = reflect_set(obj, propName, value, receiver);
                    return success;
                }
    ...
    function createSuper(obj, superTarget)
    {
        ...
        {
            get: (target, propName) =>
                resolve(() => reflect_get(obj, propName, superTarget)),
            set: (target, propName, value) =>
                resolve(() => reflect_set(obj, propName, value, superTarget))
        };
    ...
    function invalidSuperOperation()
    {
        throw new ReferenceError('Invalid operation involving superclass');
    }
