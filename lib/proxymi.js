...
(global =>
{
    ...
    function checkDuplicateSuperType(typeMap, type)
    {
        if (typeMap.has(type))
        {
    ...
    function classesImpl(types)
    {
        ...
        {
            checkDuplicateSuperType(typeToPrototypeMap, type);
    ...
    function createConstructorProxy(types, typeToPrototypeMap)
    {
        const constructorTarget =
            function (...args)
            {
                const typeToSuperArgsMap =
                    createTypeToSuperArgsMap(types, typeToPrototypeMap, args);
        ...
        const constructorProperties =
        {
            ...
            name: describeGetter(() => `(${types.map(type => nameOfType(type))})`),
    ...
    function createSuperPrototypeCall(superPrototypeTarget, typeToPrototypeMap)
    {
        const superPrototypeCall =
        {
            class(type)
            {
                ...
                if (prototype === null)
                    throw TypeError('Property \'prototype\' of superclass is null');
    ...
    function createTypeToSuperArgsMap(types, typeToPrototypeMap, args)
    {
